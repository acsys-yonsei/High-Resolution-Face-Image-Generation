...
def parse_args():
  ...
  parser.add_argument('--steps', type=int, default=10,
                      help='Number of steps(from boundary) for image editing. (default: 10)')
...
def main():
  ...
  gan_type = 'ganformer' #not implemented anyway
  if gan_type == 'pggan':
  ...
  if os.path.isfile(args.input_latent_codes_path):
    ...
    latent_codes = latent_codes[:args.max]
    print(latent_codes.shape)
    dim0, dim1, dim2 = latent_codes.shape[0], latent_codes.shape[-1], latent_codes.shape[-2]
    latent_codes = latent_codes.reshape(dim0, dim1*dim2)
  ...
  latents_all = np.empty((0,dim2,dim1),int)
  for sample_id in tqdm(range(total_num), leave=False):
    interpolations = linear_interpolate(latent_codes[sample_id:sample_id + 1],
                                        ...
                                        steps=args.steps)
    interpolation_id = 0
    latents = interpolations.reshape(args.steps,dim2, dim1)
    latents_all = np.append(latents_all, latents,axis=0)
   
  #latents_all = latents.reshape(total_num, dim2, dim1)
  print("saving edited latents {}".format(latents_all.shape))
  np.save(args.output_dir+"/edited_latents.npy",latents_all)
